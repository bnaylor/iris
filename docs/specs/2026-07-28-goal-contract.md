...
- **Goal state gains the contract; the loop gate is unchanged.** `Conversation` gains `goalContract: GoalContract?`. The loop's "is a goal active" gate **stays `activeGoal != nil`** — because subagents also drive the auto-reprompt via `activeGoal` (no contract), and all of #16's machinery is gated on it. Locking a contract sets `activeGoal = objective`, keeping the gate valid for both main-agent goals (with a contract) and subagents (without). Invariant: a locked contract mirrors its `objective` into `activeGoal`; a legacy persisted `activeGoal` with no contract is upgraded on load to a locked single-`qualitative`-criterion contract. The contract is the *additional oracle*, not a replacement for the gate.
...
- **Migration hazard is sidestepped, not embraced.** Because the loop gate stays `activeGoal != nil` (above), the ~six `activeGoal` read sites (loop detection, iteration cap, auto-reprompt, soft-stop in `iris.swift`; subagent kickoff in `SubagentManager`) are **not** converted — they keep working unchanged. The only new invariant to test is that locking a contract sets `activeGoal`, and that a legacy `activeGoal`-only conversation loads as a locked contract. This is the lower-risk path and the reason #16 and subagents are unaffected.
